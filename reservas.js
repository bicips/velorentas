...
function toast(m,ms,tipo){
  ms=ms||2600;
  var t=document.getElementById('toast');
  if(!t)return;
  var bg=tipo==='error'?'#dc2626':tipo==='success'?'#16a34a':'#1f2937';
  t.textContent=m;
  t.style.cssText='position:fixed;bottom:24px;left:50%;transform:translateX(-50%);background:'+bg+';color:#fff;padding:12px 24px;border-radius:10px;font-size:14px;font-weight:700;z-index:99999;box-shadow:0 4px 16px rgba(0,0,0,.3);white-space:nowrap;display:block';
  setTimeout(function(){t.style.display='none';},ms);
}
...
var SCFG={
  ...
  // Estados legacy (datos antiguos) — liberan stock igual que recogida
  activa:           {lbl:'Activa (legacy)',    cls:'bn-green',  icon:'🔄'},
  finalizada:       {lbl:'Finalizada',         cls:'bn-gray',   icon:'🏁'}
};
// Estados que LIBERAN stock — única fuente de verdad, usada en toda la app
var ESTADOS_LIBRE_GLOBAL=['cancelada','anulada','recogida','finalizada'];
...
function bikeOperativa(b){return b.estado!=='averiada'&&b.estado!=='baja';} // stock = todas menos averiadas/baja

function bikeFreeForRange(bikeId,ini,fin,excludeId){var ok=true;reservas.forEach(function(r){
  if(ESTADOS_LIBRE_GLOBAL.indexOf(r.estado)>=0||(excludeId&&r.id===excludeId))return;
  // FIX: una reserva puede tener VARIAS bicis asignadas (bikesAsig / lineas[].bikeAsig), no solo r.bikeId
  // (que es solo la primera, por compatibilidad). Antes solo se comprobaba r.bikeId y la 2a bici en
  // adelante de una reserva multi-bici se consideraba erróneamente "libre".
  var tieneEstaBici=(r.bikeId===bikeId)
    ||(r.bikesAsig&&r.bikesAsig.some(function(a){return a.id===bikeId;}))
    ||(r.lineas&&r.lineas.some(function(l){return l.bikeAsig&&l.bikeAsig.id===bikeId;}));
  if(!tieneEstaBici)return;
  var g=(r.prepDias!=null?r.prepDias:cfg.gapDays);
  var rIniG=addD(r.ini,-g), rFinG=addD(r.fin,g);
  if(!(fin<rIniG||ini>rFinG))ok=false;});return ok;}

// FIX: unidades de una línea ya cubiertas por asignación manual de bici concreta y libre de verdad.
// El número de línea del timeline (fila 1, 2...) es solo un índice visual — NO identifica la bici física.
// La bici real es la que está en `asignadas` (bikesAsig / l.bikeAsig), comprobada bici a bici con bikeFreeForRange.
function udsAsignadasLibres(tipo,talla,ini,fin,excludeId,listaAsignadas){
  var lista=listaAsignadas||asignadas||[];
  return lista.filter(function(a){
    return a.tipo===tipo && a.talla===talla && bikeFreeForRange(a.id,ini,fin,excludeId);
  }).length;
}
function getAvailCount(tipo,talla,ini,fin,excludeId){
  var niG=addD(ini,-cfg.gapDays),finG=addD(fin,cfg.gapDays); // gap por defecto (bloqueos genéricos)
  var blqUds=0;
  bloqueos.forEach(function(b){if(b.tipo!==tipo||b.talla!==talla)return;if(!(fin<b.ini||niG>b.fin))blqUds+=(b.uds||1);});
  // Total bicis del tipo+talla en flota (no averiadas)
  var total=bikes.filter(function(b){return b.tipo===tipo&&b.talla===talla&&bikeOperativa(b);}).length;
  // Restar unidades comprometidas en reservas activas con fechas solapadas
  // (tanto las que tienen bici asignada como las que no — la reserva ya ocupa stock)
  var ESTADOS_LIBRE=ESTADOS_LIBRE_GLOBAL;
  var udsOcupadas=0;
  reservas.forEach(function(r){
    if(ESTADOS_LIBRE.indexOf(r.estado)>=0)return;
    if(excludeId&&r.id===excludeId)return;
    // FIX: respeta los días de preparación fijados manualmente para ESTA reserva en el timeline
    // (r.prepDias, editable con el botón 🔧 del timeline) — antes solo era decorativo
    var g=(r.prepDias!=null?r.prepDias:cfg.gapDays);
    var rIniG=addD(r.ini,-g), rFinG=addD(r.fin,g);
    if(fin<rIniG||ini>rFinG)return; // fechas no solapan
    if(r.lineas&&r.lineas.length){
      r.lineas.forEach(function(l){
        if(l.tipo===tipo&&l.talla===talla) udsOcupadas+=(l.uds||1);
      });
    } else if(r.tipo===tipo&&r.talla===talla){
      udsOcupadas+=(r.uds||1);
    }
  });
  return Math.max(0,total-blqUds-udsOcupadas);
}
function getAvailBikes(tipo,talla,ini,fin,excludeId){
  var niG=addD(ini,-cfg.gapDays),finG=addD(fin,cfg.gapDays);
  var blqUds=0;
  bloqueos.forEach(function(b){if(b.tipo!==tipo||b.talla!==talla)return;if(!(fin<b.ini||niG>b.fin))blqUds+=(b.uds||1);});
  var todasDelTipo=bikes.filter(function(b){return b.tipo===tipo&&b.talla===talla&&bikeOperativa(b);});
  var ESTADOS_LIBRE=ESTADOS_LIBRE_GLOBAL;
  var ocupadasIds={};
  reservas.forEach(function(r){
    if(ESTADOS_LIBRE.indexOf(r.estado)>=0)return;
    ...
    var g=(r.prepDias!=null?r.prepDias:cfg.gapDays);
    var rIniG=addD(r.ini,-g), rFinG=addD(r.fin,g);
    if(!(fin<rIniG||ini>rFinG)){
      if(r.lineas&&r.lineas.length){
        r.lineas.forEach(function(l){if(l.bikeAsig)ocupadasIds[l.bikeAsig.id]=true;});
      }
      if(r.bikesAsig&&r.bikesAsig.length){
        r.bikesAsig.forEach(function(a){ocupadasIds[a.id]=true;});
      }
    }
  });
  var free=todasDelTipo.filter(function(b){return !ocupadasIds[b.id];});
  return free.slice(blqUds);
}
...
function renderDash(){
  ...
  var ingr=reservas.filter(function(r){return r.estado!=='cancelada'&&r.estado!=='anulada'&&r.ini.slice(0,7)===mes;}).reduce(function(s,r){return s+(r.total||0);},0);
...
function renderLineas(){
  ...
  lineas.forEach(function(l,i){
    ...
    if(l.tipo&&l.talla&&ini&&fin&&diffD(ini,fin)+1>=cfg.minDays){
      ...
      // FIX: si ya hay bicis asignadas manualmente y libres para esta línea, cuentan aparte del stock genérico
      var udsAsig=udsAsignadasLibres(l.tipo,l.talla,ini,fin,eidN);
      var efectivo=cnt+udsAsig;
      availTxt=efectivo>=l.uds
        ?'<span style="font-size:11px;color:#16a34a;font-weight:700;white-space:nowrap">✅ '+efectivo+' disp.'+(udsAsig?' ('+udsAsig+' asig.)':'')+'</span>'
        :'<span style="font-size:11px;color:#dc2626;font-weight:700;white-space:nowrap">❌ '+efectivo+' disp.</span>';
    }
    ...
    html+='</div>';
    html+=availTxt;
    // Bici asignada a esta línea (guardada en l.bikeAsig)
    if(l.bikeAsig){
      html+='<span style="font-size:11px;font-weight:700;color:#0f766e;background:#ccfbf1;padding:3px 8px;border-radius:6px;white-space:nowrap">🚲 '+esc(l.bikeAsig.num)+'</span>';
  ...
  el.querySelectorAll('select[data-li],input[data-li]').forEach(function(inp){
    inp.addEventListener('change',function(){
      ...
      if(field==='talla'){
        lineas[idx].talla=this.value;
        var p=tarifas[lineas[idx].tipo+'_'+this.value];if(p)lineas[idx].ppd=p;
        // Comprobar si hay más líneas de este tipo+talla que stock disponible
        var iniV=document.getElementById('r-ini').value,finV=document.getElementById('r-fin').value;
        var eidV=document.getElementById('r-eid').value,eidNv=eidV?parseInt(eidV):null;
        if(lineas[idx].tipo&&lineas[idx].talla&&iniV&&finV){
          var cntDisp=getAvailCount(lineas[idx].tipo,lineas[idx].talla,iniV,finV,eidNv);
          var cntLineas=lineas.filter(function(l){return l.tipo===lineas[idx].tipo&&l.talla===lineas[idx].talla;}).length;
          if(cntLineas>cntDisp){
            var t=tipos[lineas[idx].tipo]||{label:lineas[idx].tipo};
            toast('⚠️ Solo hay '+cntDisp+' '+t.label+' '+lineas[idx].talla+' disponibles, tienes '+cntLineas+' líneas',3500,'error');
          }
        }
        renderLineas();recalcPPD();return;
      }
      if(field==='uds'){
        lineas[idx].uds=parseInt(this.value)||1;
        var iniU=document.getElementById('r-ini').value,finU=document.getElementById('r-fin').value;
        var eidU=document.getElementById('r-eid').value,eidNu=eidU?parseInt(eidU):null;
        if(lineas[idx].tipo&&lineas[idx].talla&&iniU&&finU){
          var cntDispU=getAvailCount(lineas[idx].tipo,lineas[idx].talla,iniU,finU,eidNu);
          var totalUdsLineas=lineas.filter(function(l){return l.tipo===lineas[idx].tipo&&l.talla===lineas[idx].talla;}).reduce(function(s,l){return s+(l.uds||1);},0);
          if(totalUdsLineas>cntDispU){
            var tU=tipos[lineas[idx].tipo]||{label:lineas[idx].tipo};
            toast('⚠️ Solo hay '+cntDispU+' '+tU.label+' '+lineas[idx].talla+' disponibles, pides '+totalUdsLineas,3500,'error');
          }
        }
        renderLineas();recalcPPD();return;
      }
  ...
  el.querySelectorAll('button[data-rm-asig]').forEach(function(btn){
    btn.addEventListener('click',function(){
      ...
      if(lineas[idx] && lineas[idx].bikeAsig){
        // Liberar la bici en la flota
        var bikeId = lineas[idx].bikeAsig.id;
        var bikeObj = bikes.find(function(b){ return b.id===bikeId; });
        if(bikeObj){
          bikeObj.estado='disponible';
          DB.saveBici(bikeObj).catch(function(e){ console.error('liberarBici',e); });
        }
        delete lineas[idx].bikeAsig;
      }
      _syncAsignadas();
      renderLineas();renderAsig();
      // Abrir modal de asignación manual para esa línea automáticamente
      _lineaAsigIdx=idx;
      renderManList();
      openM('mmanasign');
    });
...
function checkAllLineas(){
  ...
  lineas.forEach(function(l){if(!l.tipo||!l.talla)return;var cnt=getAvailCount(l.tipo,l.talla,ini,fin,eidN);var udsAsig=udsAsignadasLibres(l.tipo,l.talla,ini,fin,eidN);var efectivo=cnt+udsAsig;var t=tipos[l.tipo]||{label:l.tipo};if(efectivo<l.uds){hasError=true;msgs.push('❌ '+t.label+' '+l.talla+': solo '+efectivo+' disp., necesitas '+l.uds);}else{msgs.push('✅ '+t.label+' '+l.talla+' ×'+l.uds+': disponible'+(udsAsig?' ('+udsAsig+' ya asignada'+(udsAsig!==1?'s':'')+')':''));}});
...
function openNewRes(preTipo,preTalla){
  ...
  openM('mres');
  renderLineas();renderAsig();initExtrasModal();
}
...
function openEditRes(id){
  ...
  // Usar lineas[i].bikeAsig directamente si ya viene de Supabase
  // Solo reconstruir desde bikesAsig para reservas antiguas sin bikeAsig en líneas
  var tieneLineasConAsig = rLineas.some(function(l){ return l.bikeAsig; });
  if(!tieneLineasConAsig && r.bikesAsig && r.bikesAsig.length){
    // Formato antiguo: reconstruir desde bikesAsig (una bici por línea en orden)
    var rawAsig = r.bikesAsig.slice();
    var usados = {};
    rawAsig.forEach(function(a){
      for(var i=0;i<rLineas.length;i++){
        if(!usados[i] && rLineas[i].tipo===a.tipo && rLineas[i].talla===a.talla){
          rLineas[i].bikeAsig={id:a.id,num:a.num,tipo:a.tipo,talla:a.talla};
          usados[i]=true;
          break;
        }
      }
  ...
  _syncAsignadas();
  document.getElementById('r-dinfo').style.display='none';
  document.getElementById('r-fact-btn').style.display=(['confirmada','proxima_entrega','en_preparacion','preparada','transito_furgo','transito_carlos','transito_seur','entregada','alquiler','pendiente_recoger','recogida','finalizada'].indexOf(r.estado)>=0)?'inline-flex':'none';
  calcDays();
  openM('mres');
  renderLineas();renderAsig();loadExtrasFromRes(r);
}

function fillTipoSel(){}

// ════════════════════════════════════════════════════
//  GESTIÓN DE TIPOS Y MODELOS
// ════════════════════════════════════════════════════
function saveTipos(){
  DB.saveConfig('tipos', tipos).catch(function(e){ console.error('saveTipos', e); });
}
...
function renderModelosList(){
  var el=document.getElementById('modelos-list');
  if(!el)return;
  var keys=Object.keys(tipos);
  if(!keys.length){
    el.innerHTML='<div style="padding:20px;text-align:center;color:var(--g400);font-size:13px">No hay modelos definidos</div>';
    return;
  }
  el.innerHTML=keys.map(function(k){
    var t=tipos[k];
    var nBikes=bikes.filter(function(b){return b.tipo===k;}).length;
    return '<div style="display:flex;align-items:center;gap:10px;padding:10px 12px;border:1px solid var(--g200);border-radius:8px;margin-bottom:8px;background:#fff">'
      +'<div style="width:36px;height:36px;border-radius:8px;background:'+t.color+'22;display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0">'+t.icon+'</div>'
      +'<div style="flex:1;min-width:0">'
        +'<div style="font-weight:700;font-size:14px">'+esc(t.label)+'</div>'
        +'<div style="font-size:11px;color:var(--g400)">clave: '+esc(k)+' · tallas: '+t.sizes.join(', ')+' · '+nBikes+' bici'+(nBikes!==1?'s':'')+'</div>'
      +'</div>'
      +'<button class="btn bs bxs" onclick="openEditModelo(\''+k+'\')">✏️</button>'
      +'<button class="btn bd bxs" onclick="deleteModelo(\''+k+'\')">🗑️</button>'
    +'</div>';
  }).join('');
}

function openNuevoModelo(){
  document.getElementById('mmodelo-edit-tit').textContent='Nuevo modelo';
  document.getElementById('mod-key-orig').value='';
  document.getElementById('mod-key').value='';
  document.getElementById('mod-key').disabled=false;
  document.getElementById('mod-label').value='';
  document.getElementById('mod-icon').value='🚲';
  document.getElementById('mod-color').value='#3b82f6';
  document.getElementById('mod-sizes').value='S,M,L';
  openM('mmodelo-edit');
}

function openEditModelo(key){
  var t=tipos[key];
  if(!t)return;
  document.getElementById('mmodelo-edit-tit').textContent='Editar modelo';
  document.getElementById('mod-key-orig').value=key;
  document.getElementById('mod-key').value=key;
  document.getElementById('mod-key').disabled=true; // no permitir cambiar la clave de un modelo existente
  document.getElementById('mod-label').value=t.label;
  document.getElementById('mod-icon').value=t.icon;
  document.getElementById('mod-color').value=t.color;
  document.getElementById('mod-sizes').value=t.sizes.join(',');
  openM('mmodelo-edit');
}

function saveModelo(){
  var keyOrig=document.getElementById('mod-key-orig').value;
  var key=document.getElementById('mod-key').value.trim().toLowerCase().replace(/[^a-z0-9_]/g,'');
  var label=document.getElementById('mod-label').value.trim();
  var icon=document.getElementById('mod-icon').value.trim()||'🚲';
  var color=document.getElementById('mod-color').value;
  var sizes=document.getElementById('mod-sizes').value.split(',').map(function(s){return s.trim();}).filter(Boolean);

  if(!key){toast('La clave interna es obligatoria');return;}
  if(!label){toast('El nombre visible es obligatorio');return;}
  if(!sizes.length){toast('Añade al menos una talla');return;}
  if(!keyOrig && tipos[key]){toast('Ya existe un modelo con esa clave');return;}

  tipos[key]={label:label,icon:icon,color:color,sizes:sizes};
  saveTipos();
  closeM('mmodelo-edit');
  renderModelosList();
  initFlotaSelects();
...
function deleteModelo(key){
  var nBikes=bikes.filter(function(b){return b.tipo===key;}).length;
  if(nBikes>0){
    toast('❌ No se puede eliminar: hay '+nBikes+' bicicleta'+(nBikes!==1?'s':'')+' con este modelo');
    return;
  }
  if(!confirm('¿Eliminar el modelo "'+tipos[key].label+'"? Esta acción no se puede deshacer.'))return;
  delete tipos[key];
  saveTipos();
  renderModelosList();
  initFlotaSelects();
...
function exportarModelosCSV(){
  var headers=['clave','nombre','icono','color','tallas'];
  var rows=Object.keys(tipos).map(function(k){
    var t=tipos[k];
    return [k,t.label,t.icon,t.color,t.sizes.join('|')];
  });
  ...
  a.href=url;
  a.download='modelos_velorentas_'+new Date().toISOString().slice(0,10)+'.csv';
...
function importarModelosCSV(event){
  ...
  reader.onload=function(e){
    try{
      ...
      if(lines.length<2){toast('CSV vacío o sin datos');return;}
      var added=0, updated=0, errors=[];
      for(var i=1;i<lines.length;i++){
        var cols=_parseCSVLine(lines[i]);
        if(cols.length<5)continue;
        var key=(cols[0]||'').trim().toLowerCase().replace(/[^a-z0-9_]/g,'');
        var label=(cols[1]||'').trim();
        var icon=(cols[2]||'🚲').trim();
        var color=(cols[3]||'#3b82f6').trim();
        var sizes=(cols[4]||'').split('|').map(function(s){return s.trim();}).filter(Boolean);
        if(!key||!label||!sizes.length){errors.push('Fila '+(i+1)+': datos incompletos');continue;}
        if(tipos[key])updated++;else added++;
        tipos[key]={label:label,icon:icon,color:color,sizes:sizes};
      }
      saveTipos();
      renderModelosList();
      initFlotaSelects();
      var msg='✅ Importado: '+added+' nuevos, '+updated+' actualizados';
...
function saveRes(){
  ...
  var eid=document.getElementById('r-eid').value,eidN=eid?parseInt(eid):null;
  var errStock=[];
  validLineas.forEach(function(l){
    var cnt=getAvailCount(l.tipo,l.talla,ini,fin,eidN);
    // FIX: el número de línea del timeline es solo un índice visual, no identifica una bici física.
    // Si ya has asignado manualmente una bici concreta (real) y está libre, esa unidad cuenta aparte
    // del conteo genérico por tipo+talla — antes se ignoraba y bloqueaba el guardado igualmente.
    var udsAsig=udsAsignadasLibres(l.tipo,l.talla,ini,fin,eidN);
    var efectivo=cnt+udsAsig;
    var t=tipos[l.tipo]||{label:l.tipo};
    if(efectivo<(l.uds||1)){
      errStock.push({label:t.label,talla:l.talla,pide:l.uds||1,disp:efectivo});
    }
  });
  if(errStock.length){
    var msg=errStock.map(function(e){
      return '• '+e.label+' talla '+e.talla+': pides '+e.pide+', solo hay '+e.disp+' disponible'+(e.disp!==1?'s':'');
    }).join('\n');
    ...
    modal.innerHTML='<div style="background:#fff;border-radius:12px;width:400px;max-width:92vw;padding:24px;box-shadow:0 20px 60px rgba(0,0,0,.3)">'
      +'<div style="font-size:32px;text-align:center;margin-bottom:10px">🚫</div>'
      +'<div style="font-size:16px;font-weight:800;text-align:center;margin-bottom:8px;color:#dc2626">Sin stock suficiente</div>'
      +'<div style="font-size:13px;color:#6b7280;text-align:center;margin-bottom:16px">No se puede guardar la reserva:</div>'
      +'<div style="background:#fef2f2;border:1px solid #fecaca;border-radius:8px;padding:12px 16px;margin-bottom:20px;font-size:13px;font-family:monospace;white-space:pre-line;color:#dc2626">'+msg+'</div>'
      +'<button id="btn-stock-err" style="width:100%;padding:10px;background:#dc2626;color:#fff;border:none;border-radius:8px;font-size:14px;font-weight:700;cursor:pointer">Entendido — cerrar</button>'
      +'</div>';
    document.body.appendChild(modal);
    document.getElementById('btn-stock-err').onclick=function(){modal.remove();};
    return;
  }
  var prim=validLineas[0];
  // FIX: la asignación de bici es SIEMPRE manual e informativa — no auto-asignar la primera libre.
  // bikeId/bikeNum se derivan solo de una asignación real (lineas[].bikeAsig).
  var primAsig=null;
  lineas.forEach(function(l){if(!primAsig&&l.bikeAsig)primAsig=l.bikeAsig;});
  var bikeId=primAsig?primAsig.id:null;
  var bikeNum=primAsig?primAsig.num:'';
...
function assignBike(bike){
  if(_lineaAsigIdx===null){
    // Flujo QR/global: asignar a primera línea sin bici del mismo tipo+talla
    var li=lineas.findIndex(function(l){
      return l.tipo===bike.tipo && l.talla===bike.talla && !l.bikeAsig;
    });
    if(li>=0){
      lineas[li].bikeAsig={id:bike.id,num:bike.numBici,tipo:bike.tipo,talla:bike.talla};
    } else {
      // Si no hay línea libre, simplemente añadir al array legacy
      if(asignadas.find(function(a){return a.id===bike.id;})){
        toast(esc(bike.numBici)+' ya está asignada');return;
      }
      asignadas.push({id:bike.id,num:bike.numBici,tipo:bike.tipo,talla:bike.talla,modelo:bike.modelo||''});
    }
  } else {
    // Flujo por línea: asignar a la línea exacta
    lineas[_lineaAsigIdx].bikeAsig={id:bike.id,num:bike.numBici,tipo:bike.tipo,talla:bike.talla};
    _lineaAsigIdx=null;
    closeM('mmanasign');
  }
  // FIX: marcar la bici como 'asignada' en la flota (igual que hace el taller)
  if(bike.estado!=='asignada'&&bike.estado!=='averiada'){
    bike.estado='asignada';
    if(!bike.estadosHist)bike.estadosHist=[];
    bike.estadosHist.push({estado:'asignada',fecha:new Date().toLocaleString('es-ES'),ts:new Date().toISOString(),nota:'Asignación reserva'});
    DB.saveBici(bike).catch(function(e){console.error('assignBike saveBici',e);});
  }
  // Sincronizar asignadas desde lineas
  _syncAsignadas();
  renderLineas();
  renderAsig();
  ...
  // Guardar si estamos editando una reserva existente
  if(currentResId){
    var r=reservas.find(function(x){return x.id===currentResId;});
    if(r){
      r.bikesAsig=asignadas.slice();
      r.lineas=lineas.slice();
      if(asignadas.length>0){r.bikeId=asignadas[0].id;r.bikeNum=asignadas[0].num;}
      if(r.estado==='pendiente'){r.estado='confirmada';toast('🚲 '+bike.numBici+' asignada · Reserva confirmada');}
      else{toast('🚲 '+bike.numBici+' asignada y guardada');}
      sR();updBadge();
      if(CV==='res')renderRes();
...
function _syncAsignadas(){
  // Reconstruir asignadas desde lineas[].bikeAsig
  asignadas=[];
  lineas.forEach(function(l){
    if(l.bikeAsig) asignadas.push(Object.assign({},l.bikeAsig));
  });
}
...
function renderManList(){
  ...
  el.innerHTML=headerHtml+(list.length?list.map(function(b){
    ...
    reservas.forEach(function(r){
      ...
      // Comprobar en lineas[].bikeAsig (fuente de verdad) y en bikesAsig (compatibilidad)
      var tieneEstaBici = false;
      if(r.lineas && r.lineas.length){
        tieneEstaBici = r.lineas.some(function(l){ return l.bikeAsig && l.bikeAsig.id === b.id; });
      }
      if(!tieneEstaBici && r.bikesAsig && r.bikesAsig.length){
        tieneEstaBici = r.bikesAsig.some(function(a){ return a.id === b.id; });
      }
      if(tieneEstaBici) ocupadaPor = r.cliente + ' (' + fmtD(r.ini) + '–' + fmtD(r.fin) + ')';
    });
    // También comprobar en la reserva actual que estamos editando (otras líneas)
    if(!ocupadaPor && eidN){
      var resActual = reservas.find(function(r){ return r.id === eidN; });
      if(resActual && resActual.lineas){
        var lineaConBici = resActual.lineas.findIndex(function(l){ return l.bikeAsig && l.bikeAsig.id === b.id; });
        // Si está en otra línea distinta a la que estamos asignando, está ocupada
        if(lineaConBici >= 0 && lineaConBici !== _lineaAsigIdx){
          ocupadaPor = 'otra línea de esta reserva';
        }
      }
    }
    ...
    var estadoBtn='';
    if(ya){
      // Encontrar qué línea tiene esta bici para quitarla correctamente
      var lineaConBici = lineas.findIndex(function(l){ return l.bikeAsig && l.bikeAsig.id === b.id; });
      estadoBtn='<button class="btn bd bxs" onclick="if(lineas['+lineaConBici+'])delete lineas['+lineaConBici+'].bikeAsig;_syncAsignadas();renderLineas();renderAsig();renderManList()">Quitar</button>';
    } else if(ocupadaPor){
      estadoBtn='<span style="font-size:11px;color:var(--r600);font-weight:700;text-align:right">⚠️ Ocupada:<br>'+esc(ocupadaPor)+'</span>';
    } else {
      estadoBtn='<button class="btn bb bxs" onclick="assignBike(bikes.find(function(x){return x.id==='+b.id+';}))">Asignar</button>';
    }
...
function selAll(){
  // FIX: aplicar EXACTAMENTE los mismos filtros que renderRes (incluye fechas y bikeNum)
  var q=(document.getElementById('rsrch')||{value:''}).value.toLowerCase();
  var fechaF=getFechaFilter();
  var list=reservas.filter(function(r){
    if(resF!=='all'){
...
function batchStatus(st){
  ...
  var n=0;
  var libera=ESTADOS_LIBRE_GLOBAL.indexOf(st)>=0;
  resSel.forEach(function(id){
    var r=reservas.find(function(x){return x.id===id;});
    if(r&&r.estado!==st){
      r.estado=st;n++;
      // Si el estado libera stock, liberar también las bicis asignadas
      if(libera&&r.bikesAsig&&r.bikesAsig.length){
        r.bikesAsig.forEach(function(a){
          var b=bikes.find(function(x){return x.id===a.id;});
          if(b&&b.estado!=='averiada'){b.estado='disponible';DB.saveBici(b).catch(function(e){console.error(e);});}
        });
      }
    }
  });
  ...
  if(CV==='flota')renderFlota();
  var sc=SCFG[st]||{lbl:st};
  toast(n+' reservas → '+sc.lbl);
}
function batchDelete(){
  if(!resSel.length)return;
  if(!window.confirm('Eliminar '+resSel.length+' reserva'+(resSel.length!==1?'s':'')+' seleccionada'+(resSel.length!==1?'s':'')+'?'))return;
  var aBorrar=reservas.filter(function(r){return resSel.indexOf(r.id)>=0;});
  aBorrar.forEach(function(r){
    // Liberar bicis asignadas en la flota
    if(r.lineas){r.lineas.forEach(function(l){if(l.bikeAsig){var b=bikes.find(function(x){return x.id===l.bikeAsig.id;});if(b&&b.estado==='asignada'){b.estado='disponible';DB.saveBici(b).catch(function(e){console.error(e);});}}});}
    // FIX: borrar de Supabase — sR() es upsert y NO elimina filas
    DB.deleteReserva(r.id).catch(function(e){console.error('batchDelete deleteReserva',e);});
  });
  reservas=reservas.filter(function(r){return resSel.indexOf(r.id)<0;});
  resSel=[];renderRes();renderDash();updBadge();
  if(CV==='cal')renderCal();if(CV==='timeline')renderTL();
...
function delRes(id){
  var r=reservas.find(function(x){return x.id===id;});
  if(!r||!window.confirm('Eliminar reserva de '+r.cliente+'?'))return;
  // Liberar bicis asignadas en la flota
  if(r.lineas){r.lineas.forEach(function(l){if(l.bikeAsig){var b=bikes.find(function(x){return x.id===l.bikeAsig.id;});if(b&&b.estado==='asignada'){b.estado='disponible';DB.saveBici(b).catch(function(e){console.error(e);});}}});}
  reservas=reservas.filter(function(x){return x.id!==id;});
  // Borrar de Supabase directamente (no upsert)
  DB.deleteReserva(id).catch(function(e){console.error('deleteReserva',e);});
  renderRes();updBadge();
  if(CV==='dash')renderDash();if(CV==='cal')renderCal();if(CV==='timeline')renderTL();
...
function changeStatus(id,st,extra){
  ...
  // FIX: liberar bicis en CUALQUIER estado que libera stock (recogida, cancelada, anulada, finalizada)
  if(ESTADOS_LIBRE_GLOBAL.indexOf(st)>=0&&r.bikesAsig&&r.bikesAsig.length){
    r.bikesAsig.forEach(function(a){
      var b=bikes.find(function(x){return x.id===a.id;});
      if(b&&b.estado!=='averiada'){b.estado='disponible';DB.saveBici(b).catch(function(e){console.error(e);});}
    });
...
function renderTL(){
  var tipo=document.getElementById('tl-tipo').value;
  var talla=document.getElementById('tl-talla').value;
  var now=new Date(), dates=[];
  // Mostrar desde 3 días antes hasta final del año en curso
  var endOfYear=new Date(now.getFullYear(),11,31);
  var dStart=new Date(now); dStart.setDate(dStart.getDate()-3);
  var d=new Date(dStart);
  while(d<=endOfYear){dates.push(isoD(d));d.setDate(d.getDate()+1);}
  var today2=isoD(now);
  var el=document.getElementById('tlcont');if(!el)return;

  // Bicis del modelo+talla seleccionado
  var bList=bikes.filter(function(b){
    if(!bikeOperativa(b)&&b.estado!=='averiada')return false;
    if(tipo&&b.tipo!==tipo)return false;
    if(talla&&b.talla!==talla)return false;
    return true;
  }).sort(function(a,b){return a.numBici>b.numBici?1:-1;});

  if(!bList.length){el.innerHTML=emptyHTML('🚲','Sin bicicletas para este filtro');return;}

  // Reservas activas del mismo modelo+talla
  var ESTADOS_LIBRES=ESTADOS_LIBRE_GLOBAL;
  var resActivas=reservas.filter(function(r){
    if(ESTADOS_LIBRES.indexOf(r.estado)>=0)return false;
    if(!r.lineas||!r.lineas.length)return (r.tipo===tipo)&&(!talla||r.talla===talla);
    return r.lineas.some(function(l){
      return (!tipo||l.tipo===tipo)&&(!talla||l.talla===talla);
    });
  });

  // Para cada bici, asignar reservas que no se solapen (primera bici libre)
  // Estructura: bikeId -> [reservas asignadas a esa bici para el timeline]
  var bikeRes={};
  bList.forEach(function(b){bikeRes[b.id]=[];});

  // Distribuir reservas en filas de bicis disponibles
  // Cada línea de reserva se asigna a una fila de su tipo+talla exacto
  resActivas.forEach(function(r){
    // Obtener líneas del tipo+talla del filtro actual
    var lineasADistribuir=[];
    if(r.lineas&&r.lineas.length){
      r.lineas.forEach(function(l){
        if((!tipo||l.tipo===tipo)&&(!talla||l.talla===talla)){
          for(var u=0;u<(l.uds||1);u++) lineasADistribuir.push({tipo:l.tipo,talla:l.talla});
        }
      });
    } else if(!tipo||r.tipo===tipo){
      if(!talla||r.talla===talla){
        for(var u=0;u<(r.uds||1);u++) lineasADistribuir.push({tipo:r.tipo,talla:r.talla});
      }
    }
    // Asignar cada unidad a la primera bici libre del tipo+talla correcto
    lineasADistribuir.forEach(function(li){
      for(var i=0;i<bList.length;i++){
        var bike=bList[i];
        // La bici debe coincidir en tipo+talla con la línea
        if(bike.tipo!==li.tipo||bike.talla!==li.talla)continue;
        // ¿Está libre en estas fechas?
        var libre=!bikeRes[bike.id].some(function(er){
          return !(r.fin<er.ini||r.ini>er.fin);
        });
        if(libre){
          bikeRes[bike.id].push(r);
          break;
        }
      }
    });
  ...
  // Header de fechas
  var html='<div style="overflow-x:auto"><div class="tl-wrap"><div class="tl-hdr">'
    +'<div class="tl-bcol">Bicicleta</div>'
    +'<div class="tl-dhdr">'+dates.map(function(d){
      var dt=toD(d);var isT=d===today2;
      return '<div class="tl-dh '+(isT?'tday':'')+'"><div>'+dt.getDate()+'</div>'
        +'<div style="font-size:9px">'+['L','M','X','J','V','S','D'][(dt.getDay()+6)%7]+'</div></div>';
    }).join('')+'</div></div>';

  // Filas por bici
  bList.forEach(function(bike){
    var esAveriada=bike.estado==='averiada';
    var bRes=bikeRes[bike.id]||[];
    var bBlq=bloqueos.filter(function(b){return b.tipo===bike.tipo&&b.talla===bike.talla;});
    var rowBg=esAveriada?'background:#fff1f2':'';

    html+='<div class="tl-row"><div class="tl-bc" style="'+rowBg+'">'
      +'<div class="tl-bn">'+(esAveriada?'🔧 ':'')+esc(bike.numBici)+'</div>'
      +'<div class="tl-bm">'+esc(bike.talla)+' - '+esc(bike.modelo||'')+'</div>'
      +'</div><div class="tl-cells">';

    // Celdas de días
    dates.forEach(function(d){var isT=d===today2;html+='<div class="tl-cell '+(isT?'tday':'')+'" style="'+(esAveriada?'background:#fff1f2':'')+'"></div>';});

    // Bloqueos
    bBlq.forEach(function(bl){
      var si=dates.indexOf(bl.ini),ei=dates.indexOf(bl.fin);
      if(si<0&&ei<0)return;
      var s=Math.max(0,si<0?0:si),e=Math.min(dates.length-1,ei<0?dates.length-1:ei);
      if(s>dates.length-1)return;
      var pct=(s/dates.length)*100,w=((e-s+1)/dates.length)*100;
      html+='<div class="tl-blk" style="left:'+pct+'%;width:'+w+'%;background:#fee2e2;color:#991b1b;border:1px dashed #f87171;cursor:default">🔒 '+esc(bl.motivo||'Bloqueo')+'</div>';
    });

    // Reservas asignadas a esta fila
    bRes.forEach(function(r){
      var si=dates.indexOf(r.ini),ei=dates.indexOf(r.fin);
      if(si<0&&ei<0)return;
      var s=Math.max(0,si<0?0:si),e=Math.min(dates.length-1,ei<0?dates.length-1:ei);
      if(s>dates.length-1)return;
      // Gap de preparación
      var _rGap=(r.prepDias!=null?r.prepDias:cfg.gapDays);
      var gs=e+1,ge=Math.min(dates.length-1,e+_rGap);
      if(gs<=dates.length-1){
        var gp=(gs/dates.length)*100,gw=((ge-gs+1)/dates.length)*100;
        html+='<div class="tl-blk" style="left:'+gp+'%;width:'+gw+'%;background:repeating-linear-gradient(45deg,#f3f4f6,#f3f4f6 4px,#d1d5db 4px,#d1d5db 8px);color:#6b7280;font-size:9px;border:1px dashed #9ca3af;cursor:pointer" onclick="event.stopPropagation();abrirEditPrepDias('+r.id+')" title="Preparación: '+_rGap+' días">🔧 '+_rGap+'d</div>';
      }
      var col=rcol(r.id);
      var pct=(s/dates.length)*100,w=((e-s+1)/dates.length)*100;
      html+='<div class="tl-blk" style="left:'+pct+'%;width:'+w+'%;background:'+col[0]+';color:'+col[1]+'" onclick="showResDetail('+r.id+')" title="'+esc(r.cliente)+'">'+esc(r.cliente.split(' ')[0])+'</div>';
    });
  ...
  // Leyenda
  html+='</div><div style="display:flex;gap:14px;padding:10px 14px;border-top:1px solid var(--g100);font-size:12px;color:var(--g400);flex-wrap:wrap">'
    +'<span>Leyenda:</span>'
    +'<span style="display:flex;align-items:center;gap:5px"><span style="width:12px;height:12px;background:#dbeafe;border:1px solid #1d4ed8;border-radius:3px;display:inline-block"></span>Reserva</span>'
    +'<span style="display:flex;align-items:center;gap:5px"><span style="width:12px;height:12px;background:repeating-linear-gradient(45deg,#f9fafb,#f9fafb 3px,#e5e7eb 3px,#e5e7eb 6px);border-radius:3px;display:inline-block"></span>Preparación ('+cfg.gapDays+' días)</span>'
    +'<span style="display:flex;align-items:center;gap:5px"><span style="width:12px;height:12px;background:#fee2e2;border:1px dashed #f87171;border-radius:3px;display:inline-block"></span>Bloqueado</span>'
    +'<span style="display:flex;align-items:center;gap:5px"><span style="width:12px;height:12px;background:#fff1f2;border:1px solid #fca5a5;border-radius:3px;display:inline-block"></span>Averiada</span>'
    +'</div></div>';
...
function updateInvPreview(){
  ...
  // FIX: r.total ya incluye los extras — la fila "Alquiler" debe mostrar solo la parte de bicis
  // para que la suma de filas cuadre con la base imponible (antes los extras salían duplicados)
  var extrasTotal=(r.extras||[]).reduce(function(s,e){return s+(e.precio||0)*(e.qty||1);},0);
  var baseAlquiler=Math.max(0,base-extrasTotal);
  ...
  html+='<table class="inv-table"><thead><tr><th>Descripcion</th><th>Ruta</th><th>Fechas</th><th>Dias</th><th>P/dia</th><th>Total</th></tr></thead><tbody><tr><td><strong>Alquiler '+t.icon+' '+esc(bikesLine)+'</strong><div style="font-size:11px;color:#6b7280;margin-top:2px">Bicis: '+esc(bikesAsig)+'</div></td><td style="font-size:12px">'+(lugarIni?'📍 '+esc(lugarIni)+'<br>':'')+'🏁 '+esc(lugarFin)+'</td><td style="font-size:12px">'+fmtD(r.ini)+' - '+fmtD(r.fin)+'</td><td style="text-align:center">'+r.dias+'</td><td style="text-align:right">'+(r.ppd?r.ppd.toFixed(2)+'€':'-')+'</td><td style="text-align:right;font-weight:700">'+baseAlquiler.toFixed(2)+cfg.currency+'</td></tr>'+extrasRows+'</tbody></table>';
...
function renderInformes(){
  ...
  var ingMes=reservas.filter(function(r){return r.estado!=='cancelada'&&r.estado!=='anulada'&&r.ini.slice(0,7)===mes;}).reduce(function(s,r){return s+(r.total||0);},0);
...
function _generarHojasBici(listaFiltrada) {
  // FIX: usar la lista seleccionada por el usuario, no filtrar de nuevo desde reservas[]
  var lista = listaFiltrada;
  if (!lista || !lista.length) { toast("No hay reservas para imprimir"); return; }
  ...
  // FIX: usar Blob URL siempre — funciona en GitHub Pages, móviles y con bloqueadores de popup
  try {
    var blob = new Blob([html], {type: 'text/html;charset=utf-8'});
    var blobUrl = URL.createObjectURL(blob);
    var w = window.open(blobUrl, '_blank');
    if (!w || w.closed) throw new Error("popup bloqueado");
    // Liberar el blob después de un tiempo
    setTimeout(function(){ URL.revokeObjectURL(blobUrl); }, 60000);
  } catch(e) {
    // Fallback: mostrar en iframe a pantalla completa
    var blob2 = new Blob([html], {type: 'text/html;charset=utf-8'});
    var blobUrl2 = URL.createObjectURL(blob2);
    var ov = document.createElement("div");
    ov.style.cssText = "position:fixed;inset:0;background:#fff;z-index:99999;overflow:hidden;display:flex;flex-direction:column";
    ov.innerHTML = "<div style=\"padding:8px 12px;background:#1f2937;display:flex;justify-content:space-between;align-items:center\">"
      + "<span style=\"color:#fff;font-size:13px;font-weight:700\">🖨️ Hojas de transporte — usa Ctrl+P para imprimir</span>"
      + "<button onclick=\"URL.revokeObjectURL(this.dataset.url);this.closest('div[style]').remove()\" data-url=\""+blobUrl2+"\" style=\"padding:5px 12px;background:#ef4444;color:#fff;border:none;border-radius:6px;font-size:12px;font-weight:700;cursor:pointer\">✕ Cerrar</button>"
      + "</div>"
      + "<iframe src=\""+blobUrl2+"\" style=\"flex:1;border:none;width:100%\"></iframe>";
...
function _procesarRecogida(codigo) {
  ...
  el.innerHTML = '<div style="background:#f0fdf4;border:2px solid #22c55e;border-radius:8px;padding:14px">'
    ...
    + '<label style="display:flex;align-items:center;gap:8px;margin-bottom:6px;font-size:13px;cursor:pointer">'
    + '<input type="checkbox" id="rec-chk-res" '+(res?'checked':'disabled')+'> '+(res?'Marcar reserva como <strong>Recogida</strong>':'Sin reserva activa')+'</label>'
...
function _confirmarRecogida(bikeId, resId) {
  ...
  if(resId && chkRes && chkRes.checked) {
    ...
    if(res) {
      // FIX: 'recogida' es el estado que libera stock (antes 'finalizada' bloqueaba para siempre)
      res.estado = 'recogida';
      sR();
    }
  }
  ...
  if(resId && chkRes && chkRes.checked) msgs.push('reserva recogida');
...
var STATUS_COLOR={
  pendiente:'#fef3c7',confirmada:'#dbeafe',proxima_entrega:'#ede9fe',
  en_preparacion:'#ffedd5',preparada:'#ccfbf1',
  transito_furgo:'#dbeafe',transito_carlos:'#dbeafe',transito_seur:'#dbeafe',
  entregada:'#ccfbf1',alquiler:'#dcfce7',pendiente_recoger:'#fef3c7',
  recogida:'#f3f4f6',anulada:'#fee2e2',
  activa:'#dcfce7',finalizada:'#f3f4f6',cancelada:'#fee2e2'
};
...
function imprimirReservas(){
  ...
  var SCOL=STATUS_COLOR;
  var SLBL={};Object.keys(SCFG).forEach(function(k){SLBL[k]=SCFG[k].lbl;}); // FIX: etiquetas desde SCFG (todos los estados)
...
function ejecutarImpresion(){
  ...
  var SLBL={};Object.keys(SCFG).forEach(function(k){SLBL[k]=SCFG[k].lbl;}); // FIX: todos los estados
  var SCOL2=STATUS_COLOR;
...
async function initApp() {
  ...
  try {
    ...
    if (!bikes.length && !reservas.length) {
      // DB vacía — empezar limpio, sin datos demo
    }
...
// FIX: setTimeout(0) en vez de 800ms — garantiza que taller-merged.js (que se carga después)
// ya esté parseado, pero sin retraso visible en cada carga
