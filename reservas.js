...
function bikeFreeForRange(bikeId,ini,fin,excludeId){var niG=addD(ini,-cfg.gapDays),finG=addD(fin,cfg.gapDays);var ok=true;reservas.forEach(function(r){if(r.bikeId!==bikeId||ESTADOS_LIBRE_GLOBAL.indexOf(r.estado)>=0||(excludeId&&r.id===excludeId))return;if(!(finG<r.ini||niG>r.fin))ok=false;});return ok;}
function getAvailCount(tipo,talla,ini,fin,excludeId){
  var niG=addD(ini,-cfg.gapDays),finG=addD(fin,cfg.gapDays); // gap simétrico: antes Y después
  var blqUds=0;
  ...
  reservas.forEach(function(r){
    ...
    if(finG<r.ini||niG>r.fin)return; // fechas no solapan (con gap de preparación en ambos lados)
