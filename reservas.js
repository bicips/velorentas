...
function getAvailCount(tipo,talla,ini,fin,excludeId){
  ...
  // Bicis del tipo+talla, operativas (no averiadas)
  var todasDelTipo=bikes.filter(function(b){return b.tipo===tipo&&b.talla===talla&&bikeOperativa(b);});
  // Restar las que ya están comprometidas en lineas[].bikeAsig de OTRAS reservas activas con fechas que solapan
  var ESTADOS_LIBRE=['cancelada','anulada','recogida'];
  var ocupadasIds={};
  reservas.forEach(function(r){
    if(ESTADOS_LIBRE.indexOf(r.estado)>=0)return;
    if(excludeId&&r.id===excludeId)return; // excluir la reserva que se está editando
    if(!(fin<r.ini||niG>r.fin)){ // solo si las fechas solapan
  ...
  return Math.max(0,free.length-blqUds);
}
