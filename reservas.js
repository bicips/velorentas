...
function renderTL(){
  ...
  bList.forEach(function(bike){
    ...
    bRes.forEach(function(r){var si=dates.indexOf(r.ini),ei=dates.indexOf(r.fin);if(si<0&&ei<0)return;var s=Math.max(0,si<0?0:si),e=Math.min(dates.length-1,ei<0?dates.length-1:ei);if(s>dates.length-1)return;var _rGap=(r.prepDias!=null?r.prepDias:cfg.gapDays);var gs=e+1,ge=Math.min(dates.length-1,e+_rGap);if(gs<=dates.length-1){var gp=(gs/dates.length)*100,gw=((ge-gs+1)/dates.length)*100;var prepDias=(r.prepDias!=null?r.prepDias:cfg.gapDays);html+='<div class="tl-blk" style="left:'+gp+'%;width:'+gw+'%;background:repeating-linear-gradient(45deg,#f3f4f6,#f3f4f6 4px,#d1d5db 4px,#d1d5db 8px);color:#6b7280;font-size:9px;border:1px dashed #9ca3af;cursor:pointer" onclick="event.stopPropagation();abrirEditPrepDias('+r.id+')" title="Preparación: '+prepDias+' días (clic para editar)">🔧 '+prepDias+'d</div>';}var col=rcol(r.id);var pct=(s/dates.length)*100,w=((e-s+1)/dates.length)*100;html+='<div class="tl-blk" style="left:'+pct+'%;width:'+w+'%;background:'+col[0]+';color:'+col[1]+'" onclick="showResDetail('+r.id+')" title="'+esc(r.cliente)+'">'+esc(r.cliente.split(' ')[0])+'</div>';});
    html+='</div></div>';
  });
  // Reservas confirmadas sin bici asignada — una fila por reserva
  var ESTADOS_LIBRES=['cancelada','anulada','recogida'];
  var sinAsignar=reservas.filter(function(r){
    if(ESTADOS_LIBRES.indexOf(r.estado)>=0)return false;
    if(tipo&&r.lineas){
      var tieneLinea=r.lineas.some(function(l){
        return l.tipo===tipo&&(!talla||l.talla===talla)&&!l.bikeAsig;
      });
      if(!tieneLinea)return false;
    }
    return !r.bikeId && (!r.bikesAsig||!r.bikesAsig.length) && (!r.lineas||r.lineas.every(function(l){return !l.bikeAsig;}));
  });
  if(sinAsignar.length){
    html+='<div style="border-top:2px dashed #fcd34d;margin-top:4px;padding-top:4px">';
    sinAsignar.forEach(function(r){
      var modelo=r.lineas&&r.lineas[0]?(tipos[r.lineas[0].tipo]||{label:r.lineas[0].tipo}).label+' '+r.lineas[0].talla:(r.tipo||'');
      html+='<div class="tl-row"><div class="tl-bc" style="background:#fffbeb">';
      html+='<div class="tl-bn" style="color:#92400e;font-size:11px">⚠️ '+esc(r.cliente.split(' ')[0])+'</div>';
      html+='<div class="tl-bm" style="color:#b45309">'+esc(modelo)+'</div>';
      html+='</div><div class="tl-cells">';
      dates.forEach(function(d){var isT=d===today2;html+='<div class="tl-cell '+(isT?'tday':'')+'" style="background:#fffbeb"></div>';});
      ...
      if(!(si<0&&ei<0)){
        ...
        if(s<=dates.length-1){
          ...
          html+='<div class="tl-blk" style="left:'+pct+'%;width:'+w+'%;background:#fef3c7;color:#92400e;border:1px dashed #fcd34d" onclick="showResDetail('+r.id+')" title="'+esc(r.cliente)+' — sin asignar">'+esc(r.cliente.split(' ')[0])+'</div>';
        }
      }
      html+='</div></div>';
    });
