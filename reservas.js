...
function renderTL(){
  ...
  // Para cada reserva, asignar a la primera bici libre en ese rango de fechas
  resActivas.forEach(function(r){
    // Cuántas líneas del tipo+talla tiene esta reserva
    var nLineas=1;
    if(r.lineas&&r.lineas.length){
      nLineas=r.lineas.filter(function(l){
        return (!tipo||l.tipo===tipo)&&(!talla||l.talla===talla);
      }).length||1;
    }
    // Asignar una fila por cada línea del tipo+talla
    var asignadas=0;
    for(var i=0;i<bList.length&&asignadas<nLineas;i++){
      var bike=bList[i];
