...
// La disponibilidad (getAvailCount / getAvailBikes, más abajo) se calcula SIEMPRE por
// modelo+talla: cuántas bicis de ese tipo hay en Gestión de Flota, menos las comprometidas
// por reservas que se solapan en fechas (con su margen de preparación). La asignación de
// una bici concreta a un cliente es un paso aparte, en Gestión de Flota, y no interviene
// en este cálculo ni lo modifica.
function getAvailCount(tipo,talla,ini,fin,excludeId){
  var niG=addD(ini,-cfg.gapDays); // usado solo para bloqueos manuales (mantenimiento), ver más abajo
  var blqUds=0;
  ...
  reservas.forEach(function(r){
    ...
    // FIX: el margen de preparación (r.prepDias, editable con el botón 🔧 del timeline, o por
    // defecto cfg.gapDays) se aplica SOLO hacia adelante, después de r.fin. Una reserva nueva
    // SIEMPRE puede empezar el primer día que encuentre libre — no reserva margen "por si acaso"
    // antes de que empiece otra reserva futura.
    var g=(r.prepDias!=null?r.prepDias:cfg.gapDays);
    var rFinG=addD(r.fin,g);
    if(fin<r.ini||ini>rFinG)return; // fechas no solapan
...
function getAvailBikes(tipo,talla,ini,fin,excludeId){
  var niG=addD(ini,-cfg.gapDays); // usado solo para bloqueos manuales (mantenimiento)
  var blqUds=0;
  bloqueos.forEach(function(b){if(b.tipo!==tipo||b.talla!==talla)return;if(!(fin<b.ini||niG>b.fin))blqUds+=(b.uds||1);});
  ...
  reservas.forEach(function(r){
    if(ESTADOS_LIBRE.indexOf(r.estado)>=0)return;
    if(excludeId&&r.id===excludeId)return;
    // FIX: margen de preparación solo hacia adelante (después de r.fin), nunca hacia atrás
    var g=(r.prepDias!=null?r.prepDias:cfg.gapDays);
    var rFinG=addD(r.fin,g);
    if(!(fin<r.ini||ini>rFinG)){
      if(r.lineas&&r.lineas.length){
...
function saveRes(){
  ...
  validLineas.forEach(function(l){
    // La disponibilidad se calcula SOLO por modelo+talla (conteo real de flota vs reservas
    // solapadas). La asignación de bici concreta es un paso aparte, en Gestión de Flota,
    // y no debe alterar este cálculo.
    var cnt=getAvailCount(l.tipo,l.talla,ini,fin,eidN);
    var t=tipos[l.tipo]||{label:l.tipo};
    if(cnt<(l.uds||1)){
      errStock.push({label:t.label,talla:l.talla,pide:l.uds||1,disp:cnt});
    }
  });
  if(errStock.length){
...
function renderTL(){
  ...
  // Filas por bici — FIX: se etiquetan como "PREFIJO-TALLA-NN" (p.ej. "GRA-52-01"), NUNCA con
  // el número real de bici (521, 522...) de Gestión de Flota. El timeline solo cuenta unidades
  // para calcular disponibilidad; no representa qué bici física se entrega.
  // Excepción: si la unidad está averiada, se muestra su número real para poder localizarla
  // en el taller (esa sí es información real de la flota, no una asignación de reserva).
  var _tlContador={};
  bList.forEach(function(bike,tlIdx){
    ...
    // Número de línea reiniciado en 01 para cada combinación modelo+talla
    var _grupoKey=bike.tipo+'|'+bike.talla;
    _tlContador[_grupoKey]=(_tlContador[_grupoKey]||0)+1;
    var _tInfo=tipos[bike.tipo]||{label:bike.tipo||''};
    var _prefijo=(_tInfo.label||bike.tipo||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^A-Za-z]/g,'').substring(0,3).toUpperCase();
    var etiquetaLinea=_prefijo+'-'+esc(bike.talla)+'-'+String(_tlContador[_grupoKey]).padStart(2,'0');

    html+='<div class="tl-row"><div class="tl-bc" style="'+rowBg+'">'
      +'<div class="tl-bn">'+(esAveriada?'🔧 '+esc(bike.numBici)+' (averiada)':etiquetaLinea)+'</div>'
